...
- `components`, `tokenGroups`, `themes`, `foundations`, `patterns`, `guides` — separate arrays for each entity type. All are optional.
...
The `purpose` block lists scenarios. Each use case has a `stance` of `"recommended"` (do use) or `"discouraged"` (don't use, with an alternative):
...
The `level` field, named for RFC 2119: <ds-badge>MUST</ds-badge>, <ds-badge>SHOULD</ds-badge>, <ds-badge>SHOULD_NOT</ds-badge>, <ds-badge>MUST_NOT</ds-badge>. The same values are used by `agents.constraints[].level`.
