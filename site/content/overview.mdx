...
**Draft Specification — 14 July 2026:**
...
All structured docs use one **document block** system. Each entry is a typed object with a `kind` field. The spec defines 17 block kinds — from `guidelines`, `use-cases`, and `api` to `states`, `motion`, and `accessibility`. See the [document blocks overview](/document-blocks-document-blocks) for the full list.
