...
This document defines a standard, machine-readable format for design system docs. The format covers six entity types: **components**, **design tokens** (including nested token groups), **themes**, **styles**, and **patterns**. It defines a portable, tool-agnostic structure. Software can create, edit, validate, and display design system docs using this structure.

All structured docs use a unified **guidelines** system. The guideline types are:
...
- principles, scales, and motion definitions
- content guidelines
- interactions

Each guideline is a typed container with a `type` discriminator. Entity types accept only the guideline types relevant to them. Components accept component-scoped and general types. Patterns accept pattern-scoped and general types. Styles and tokens each accept their own scoped types plus general types.
...
Each entry in `documentation` is a named collection of typed entities. Different entity types can mix freely in a single `items` array — no need to separate components from tokens from styles. Each entity identifies itself with a `type` discriminator property.
...
Every entity in the `items` array carries a `type` property that identifies its kind. The `type` is a required string const on every entity schema.
...
| `"token-group"` | A nested group of related tokens (recursive) | `entities/token.schema.json` |
...
This discriminated union model means a documentation group can hold any mix of entity types in a single flat array:
...
> **System-level vs. entity-level:** The root `purpose` and `bestPractices` properties describe the design system _as a whole_. They differ from the entity-level `purpose` and `best-practices` guideline types inside each entity's `guidelines` array. Entity-level guidelines describe when and how to use a specific component, token, or pattern. System-level properties describe when and how to use the design system itself. Both levels follow the same structure but serve different audiences and scopes.

---

## Entity Types

Every entity shares a common set of identity and metadata properties, plus a `guidelines` array for all structured docs. The `guidelines` array accepts only the guideline types suited to that entity type. Every entity also carries a required `type` discriminator (see [Entity Type Discriminators](#entity-type-discriminators)).
...
| `name` | `string` | Yes | Machine-readable identifier. Components, styles, patterns, and themes enforce `^[a-z][a-z0-9-]*$`. Token and token group names are unconstrained by design (see [Token Name Exception](#token-name-exception)). |
...
| `since` | `string` | No | The version in which this entity was introduced. |
...
| `value` | `tokenValue` | No | Display form of the token's value. See [Token Value](#token-value). |
...
A nested group of related tokens. The `children` array can hold token objects, nested token group objects, or a mix of both — forming a recursive hierarchy of any depth. Accepts **general guidelines** only.
...
Styles document foundations — visual attributes like color and spacing, plus cross-cutting concerns like accessibility, motion systems, and content rules. The `category` property classifies the domain (e.g., `"color"`, `"typography"`, `"spacing"`, `"motion"`, `"accessibility"`, `"content"`).
...
The `tokenValue` object holds a display form of a token's value for documentation tools. It is **not** the source of truth — the W3C Design Tokens Format file is.
...
| `resolution` | `string` | No | How the value was derived: `"explicit"` (direct value, not an alias) or `"alias"` (value references another token). When omitted, tools SHOULD infer: if `reference` is present, the token is an alias; otherwise it is explicit. Aligns with the W3C Design Tokens Format Module (§3.8 "Alias/Reference", §7 "Aliases/References"). |
| `dtcgFile` | `string` | No | URI or relative path to the DTCG file containing the authoritative value. |

## Token API

The `tokenApi` object maps platform names to identifier strings. You can add more platforms without schema changes. Keys are platform names; values are the identifier strings for that platform. Built-in examples:
...
Guidelines are the core unit of structured docs in DSDS. Every piece of documentation on an entity is a guideline object with a `type` discriminator. Guideline types cover:

- best practices, purpose, accessibility, examples, and content (general — all entity types)
...
Each entity type accepts only the guideline types relevant to it. The scoping rationale:

- **Components** get full structural docs. They are rendered UI elements with code-level interfaces, visual anatomy, configurable dimensions, interactive states, and measurable design specifications.
...
Documents actionable usage rules for an entity. Each item is a self-contained rule pairing guidance with a rationale. Best practices tell the reader _how_ to use the entity correctly after choosing it. For guidance on _whether_ to use it at all, see [Purpose](#purpose-purpose).
...
Documents what an entity is for — the concrete scenarios where it is the right choice and where a different entity fits better. Purpose guidelines help the reader decide _whether_ to reach for this entity before they start using it.
...
For actionable accessibility _rules_ with rationale (e.g., "Provide an aria-label for icon-only buttons"), use a best-practice entry with `category: "accessibility"` instead.
...
Documents structured content guidelines — recommended action labels and localization/i18n concerns. Content guidelines complement best-practice guidelines: best practices say "Use sentence case" (a _rule_), content guidelines say "Add: Takes an existing object and uses it in a new context" (a _reference_).
...
Documents all dimensions of visual or behavioral variation. Each item is a single axis of configuration (e.g., `"size"`, `"emphasis"`, `"full-width"`). Multiple axes document orthogonal dimensions that combine independently.
...
- **Plural names** for guideline types that wrap a list of like items in an `items` array: `"best-practices"`, `"variants"`, `"states"`, `"principles"`, `"interactions"`, `"examples"`, `"motion"`, `"content"`.
- **Singular names** for guideline types that are self-contained with their own internal structure:
...
The distinction: plural types are containers of interchangeable items where the container has no identity beyond its type. Singular types have meaningful internal structure where properties are named and semantically distinct.
...
*End of specification. See the schema files in `spec/schema/` for the normative JSON Schema definitions and the examples in `spec/examples/` for complete, validated demos of each entity and guideline type.*