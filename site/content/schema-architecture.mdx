...
| `entities/` | Entity type schemas — component, token (including tokenGroup), theme, foundation, pattern |
...
Every entity carries a `type` property that identifies its kind. The `type` is a required string const on every entity schema. Entities live in typed arrays (`components`, `tokenGroups`, `themes`, `foundations`, `patterns`), so the array name already pins the expected type. The `type` property is still required on each entity for self-description and validation.
...
| `"foundation"` | `foundations` | A broad design foundation | `entities/foundation.schema.json` |
...
```json
{
  ...
  "foundations": [
    { "type": "foundation", "identifier": "spacing", "..." }
  ],
  "patterns": [
...
| `identifier` | `string` | Yes | Machine-readable identifier. Components, foundations, patterns, and themes enforce `^[a-z][a-z0-9-]*$`. Token and token-group identifiers are unconstrained by design (see [Token Identifier Exception](#token-identifier-exception)). |
| `name` | `string` | Varies | Human-readable name. Required on component, foundation, pattern, theme. **Not present** on token or token-group — the identifier itself serves as the display label. |
| `summary` | `string` | No | One-line plain-text summary for compact display. MUST NOT contain markup. |
| `description` | `richText` | Varies | Description with CommonMark support. Required on component, foundation, pattern, theme. **Optional** on token and token-group. |
| `status` | `statusObject` | Varies | Lifecycle status with optional per-platform readiness. Required on component, foundation, pattern, theme. **Optional** on token and token-group — when omitted inside a group, inherited from the parent. |
...
| **Foundation** | foundation | principles, scale, motion + general |
...
| `"principles"` | `items` | principleEntry | Foundation | High-level guiding beliefs that shape decisions. |
| `"scale"` | `steps` | scaleStep | Foundation | Ordered progression of token values (spacing scale, type scale, etc.). |
| `"motion"` | `items` | motionEntry | Foundation | Named easing curves with cubic-bezier functions and recommended durations. |
...
| **Level 2: Complete** | Level 1 + at least one substantive document block entry (anatomy/api/variants for components, principles/scale for foundations, interactions for patterns, guideline/purpose for tokens). |