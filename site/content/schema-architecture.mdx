...
The `code` object carries the source in one of two forms: **inline** (`code` + `language`) when the source travels with the document, or **referenced** (`src` + `language`) when it lives in an external file the consumer resolves at read time. The referenced `src` is a relative path and follows the same resolution rules as a `$ref` fileRef — no build step inlines it. The flat `guidelines` array reuses the shared guideline-entry shape (guidance, rationale, level), and the `useCases` array reuses the shared use-case shape (description, stance). A chunk points to the components it composes through the `links` metadata field (with `kind: "component"`, `identifier`, and `role`).
...
An explicit checklist of items to work through for an artifact. Accepted on **component**, **foundation**, and **pattern** entities. It is built primarily for agents: a concrete list of things to verify, do, or confirm when building, reviewing, or integrating an artifact, and it reads equally well from `agentDocumentBlocks`.
