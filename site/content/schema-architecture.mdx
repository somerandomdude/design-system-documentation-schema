...
A nested group of related tokens. The `children` array can hold token objects, nested token group objects, or a mix of both — forming a recursive tree of any depth. Accepts **general guidelines** only.
...
The `tokenValue` object holds a display form of a token's value for docs tools. It is **not** the source of truth — the W3C Design Tokens Format file is.
...
Guidelines are the core unit of structured docs in DSDS. Every piece of docs on an entity is a guideline object with a `type` discriminator. Guideline types cover:
