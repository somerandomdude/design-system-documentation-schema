...
The `code` object carries the source in one of two forms: **inline** (`code` + `language`) when the source travels with the document, or **referenced** (`src` + `language`) when it lives in an external file the consumer resolves at read time. The referenced `src` is a relative path and follows the same resolution rules as a `$ref` fileRef — no build step inlines it. Structured docs live in `documentBlocks` (and agent-only rules in `agentDocumentBlocks`), both accepting the general block kinds — guidelines, useCases, accessibility, content, sections, and checklist. The flat top-level `guidelines` and `useCases` arrays are deprecated authoring shorthand for the equivalent blocks. A chunk declares the components it composes as typed `composes` edges in its `relationships` array.
...
The object form adds per-platform readiness. It also adds an optional `description` and a deprecation notice, which becomes required when `overall` is `deprecated`. An entity can be `stable` overall while some platforms are still `experimental` or `draft`.
...
| `description` | `string` | No | Optional note explaining what the status represents — the reasoning behind it, its scope, or any caveats. Separate from `deprecationNotice`, which covers only the deprecated case. |
...
**General** (available on all entity types): checklist, guidelines, useCases, accessibility, content, and sections (free-form narrative prose).

### Known naming exceptions

Two naming inconsistencies are recorded here as deliberate, pending the next major release (renames are breaking):

- The block kind `useCases` is camelCase where every other DSDS-native vocabulary value is lowercase kebab-case; it becomes `use-cases` at the next major.
- The actionable line on a checklist item is richText `label`, while a step entry uses string `title` and a scale step uses `label` where sibling entries use `name`; these converge on `label` (richText) for procedural blocks and `name` for display names at the next major.

- `systemInfo`'s `systemName`/`systemVersion` stutter against their mount point and diverge from the bare `name`/`version` style used elsewhere; they become `name`/`version` at the next major.
- The explanatory-prose property on the status object forms is `description`, while sibling metadata fields (`lastUpdated`, `lastReviewed`, `docOrigin`) use `note`; status converges on `note` at the next major.
- Sub-entry identifiers on anatomy parts, states, variant values, motion entries, and scale steps are unpatterned (values like `2xl` are legal), unlike entity identifiers — a deliberate exception, not drift.

Do not introduce new values or properties that follow the exceptions — new vocabulary MUST be lowercase kebab-case.
...
For **general documentation guidance** — rules with a rationale and a conformance level, like "Provide an aria-label for icon-only buttons" — use a `guidelines` block with `category: "accessibility"`. That is the line between the two: this block records what the artifact *does*; guidelines say what authors *should do*, and why.
...
For **general documentation guidance** — voice, tone, and writing rules with a rationale and a conformance level, like "Use sentence case" — use a `guidelines` block with `category: "content"`. That is the line between the two: a guideline says "Use sentence case" (a _rule_); a content entry says "Add: takes an existing object and uses it in a new context" (a _reference_).
...
> **Deprecated.** Using a link to express how entities relate — the relationship kinds (`"alternative"`, `"parent"`, `"child"`, `"related"`) and internal artifact references by `identifier` — is deprecated. Express these as typed edges in [Relationships](#relationships) instead. Linter rule DSDS-010 flags the deprecated forms.
