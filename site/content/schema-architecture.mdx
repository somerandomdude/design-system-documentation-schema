...
- **Document blocks** — the documentation, in the `documentBlocks` array. Two cover most needs: **`use-cases`** (when to use the thing) and **`guidelines`** (how to use it). **`sections`** holds free-form prose for anything else.
...
A pattern declares the components it uses as typed `composes` edges in its `relationships` array (with `role` describing what each component does), not through a dedicated block.
...
**General** (available on all entity types): checklist, guidelines, use-cases, accessibility, content, and sections (free-form narrative prose).

### Known naming exceptions

The naming exceptions previously recorded here were executed in the 0.14.0 breaking window: the block kind is now `use-cases`, step entries use `label` (richText), scale steps use `name`, `systemInfo` uses `name`/`version`, the status object forms use `note`, and `apiEvent` uses `payload`. One deliberate exception remains:
