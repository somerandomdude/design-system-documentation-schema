...
- **Document** — `dsdsVersion` plus either a single `entity` or a `documentation` group of entities.
- **Entity** — a `kind` (`component`, `token`, `theme`, `foundation`, `pattern`, or `guide`) with an `identifier` and a `name`.
- **Metadata** — short facts in the `metadata` array; the one you'll always use is `description`.
- **Document blocks** — the documentation, in the `documentBlocks` array. Two cover most needs: **`purpose`** (when to use the thing) and **`guideline`** (how to use it). **`section`** holds free-form prose for anything else.
...
```json
{
  ...
  "entity": {
    ...
    "name": "Button",
    "metadata": [
      { "kind": "description", "value": "Triggers an action such as submitting a form." }
    ],
    "documentBlocks": [
      {
        "kind": "purpose",
...
Everything else on this page — `anatomy`, `api`, `variants`, `states`, `design-specifications`, `motion`, `scale`, and the other metadata kinds — is **optional**. Reach for a block when you have that specific thing to document, and skip the rest. The [Quick Start](quickstart.html) is the gentlest path in; the sections below are the complete reference.
...
| `document-blocks/` | Document block type schemas — guideline, anatomy, api, import, variant, state, design-specifications, accessibility, purpose, scale, principle, motion, content, interaction, section, steps, plus the scoped union (document-blocks.schema.json) |
...
The optional `agents` property provides context intended exclusively for agentic (AI/LLM) consumption. It lives on **entities** (component, token, theme, foundation, pattern, guide) — one `agents` object per entity. (As of 0.6 it is no longer carried on individual document blocks; AI context belongs to the entity as a whole.) All properties are optional.
...
Lifecycle status is recorded as a `status` metadata entry on an entity (`kind: "status"`). It carries an `overall` status value, optional per-platform readiness, and a deprecation notice that becomes required when `overall` is `deprecated`. An entity can be `stable` overall while individual platforms are still `experimental` or `draft`.

<ds-prop-table schema="metadata/status" def="statusMetadata" />
...
- **Collection blocks** hold a list of like entries in an `items` array: `guideline`, `section`, `states`, `variants`, `steps`, `import`, `interactions`, `motion`, `principles`.
- **Three blocks keep a domain name** where it reads more clearly than `items` — `anatomy.parts`, `scale.steps`, and `purpose.useCases`. Same idea (a list of entries), just a clearer label.
...
AI/LLM context is not part of a block — it lives once on the entity, in [`agents`](#agents).
...
> **Three flavors of guidance — pick the right one:** [`purpose`](#purpose-purpose) says *whether* to use something (the right choice vs. an alternative); `guideline` says *how* to use it correctly (concrete rules + rationale); [`principles`](#principles-principles) (foundations) says *what the system believes* — the philosophy that sits above the rules.
...
The root document uses this same `purpose` block for system-level adoption guidance — see [Root document blocks](#root-document-blocks). Full model in [Use Cases](#use-cases).
...
These document block types are centered on **component** entities. `api` and `design-specifications` are component-only; `anatomy`, `variants`, and `states` are shared with patterns; and `import` is shared with guides (as noted in the [scope table](#scoped-document-block-unions)).
...
Documents high-level guiding principles that shape decisions for a domain. Principles answer "what do we believe?" and "what constraints do we accept?" — they set direction and sit *above* specific [`guideline`](#guideline-guideline)s (which encode the concrete rules).
...
The `useCases` data model provides scenario-driven guidance for when something is — and is not — the right choice. It appears wherever a `purpose` block does, with an identical shape:

1. **A root `purpose` block** ([root document blocks](#root-document-blocks)) — adoption guidance for the design system as a whole.
2. **An entity's [`purpose`](#purpose-purpose) document block** — when to reach for a specific entity.
...
- **Schema identifiers** — `$def` type names and property names — are **camelCase** (`tokenGroup`, `tokenGroups`).
...
So one concept can appear in several forms — the `token-group` kind, the `tokenGroup` type, the `tokenGroups` array — because each follows its position's rule.
