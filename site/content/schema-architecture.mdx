...
Valid `kind` values for `entity`: `"component"`, `"token"`, `"token-group"`, `"theme"`, `"foundation"`, `"pattern"`, `"guide"`, `"blueprint"`.
...
A pre-composed block of code that captures a design system pattern — a copy-paste starting point built from the system's components. Blueprints sit at the same level as components. Where a component documents one building block, a blueprint documents a composition of them, captured as code. The blueprint is intentionally simple: it does not use `documentBlocks`.

Required properties: `kind`, `identifier`, `name`, `code`.

More properties:

<ds-prop-table schema="entities/blueprint" def="blueprint" />

The `code` object holds the source (`code`) and its `language`. The flat `guidelines` array reuses the shared guideline-entry shape (guidance, rationale, level), and the `useCases` array reuses the shared use-case shape (description, stance). A blueprint points to the components it composes through the `links` metadata field (with `kind: "component"`, `identifier`, and `role`).
