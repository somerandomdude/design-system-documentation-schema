...
| `documentation` | `array` | Conditional | One or more documentation groups. Required when `entity` is absent. See [Documentation Groups](#documentation-groups). |
| `entity` | `object` | Conditional | A single entity — component, token, token-group, theme, style, or pattern. Required when `documentation` is absent. See [Single-entity documents](#single-entity-documents). |
...
| `components` | `array` | No | Component entities or `$ref` objects. Each inline item must have `"kind": "component"`. |
| `tokenGroups` | `array` | No | Token group entities or `$ref` objects. Each inline item must have `"kind": "token-group"`. |
| `themes` | `array` | No | Theme entities or `$ref` objects. Each inline item must have `"kind": "theme"`. |
| `styles` | `array` | No | Style entities or `$ref` objects. Each inline item must have `"kind": "style"`. |
| `patterns` | `array` | No | Pattern entities or `$ref` objects. Each inline item must have `"kind": "pattern"`. |
...
Each `$ref` object has one property — `$ref` — containing a URI or relative path to a DSDS file, optionally followed by a JSON Pointer fragment that targets a specific value within that file.
...
| `alternative` | `object` | No | For negative entries, a recommended option with `identifier` and `rationale`. |
...
| `constraints` | `array` | Hard rules with `must`/`must-not`/`should`/`should-not` enforcement levels. Each entry has `rule`, `level`, optional `context`, optional `evidence` (empirical backing), and optional `examples` (constraint-specific before/after code). |
| `disambiguation` | `array` | Entries resolving confusion with similar entities. Each has `entity` (name), optional `entityType` (component, prop, token, pattern, package, concept), and `distinction` (the deciding criterion). |
| `antiPatterns` | `array` | Known incorrect uses. Each has `description` (what not to do), `instead` (correct alternative — SHOULD be provided), and optional `evidence`. |
...
| `verified` | `string` (date) | ISO date when this agents block was last validated against the live design system. |
| `verifiedAgainst` | `string` | Package version, test run ID, or audit reference this block was validated against. |
...
| `tokens` | `object` | No | Token map: keys are purposes (e.g., `"background"`, `"text-color"`), values are token identifiers (e.g., `"color-action-primary"`). Same format as variant and state token references. |
...
| Property | Type | Required | Description |
|---|---|---|---|
| `identifier` | `string` | Yes | The property identifier as used in code. |
| `type` | `string` | Yes | The base type as it would appear in a type signature (e.g., `"boolean"`, `"number"`, `"string"`, `"ReactNode"`). For enum properties, use `values` instead of encoding members here. |
| `values` | `string[]` | No | Accepted values for enum-like properties (e.g., `["default", "primary", "ghost"]`). Use this instead of encoding members in `type`. |
| `description` | `richText` | Yes | What the property controls, how it affects the component, and any constraints. |
| `required` | `boolean` | Yes | Whether consumers must provide this property. |
| `defaultValue` | any | No | The default value in its native JSON type. |
| `schema` | `object` | No | Machine-readable JSON Schema (Draft 2020-12) for the property. Useful for tooling. |
| `deprecated` | `boolean` | No | Whether this property is deprecated. |
| `deprecationNotice` | `string` | No | What to use instead when deprecated. |
| `since` | `string` | No | Version in which this property was introduced. |
...
| Property | Type | Required | Description |
|---|---|---|---|
| `identifier` | `string` | Yes | The event identifier as used in code (e.g., `"onClick"`, `"onChange"`). |
| `description` | `richText` | Yes | When the event fires, what it communicates, and conditions under which it does not fire. |
| `returns` | `string` | No | The return shape (e.g., `"(event: MouseEvent) => void"`, `"{ value: string }"`). |
| `since` | `string` | No | Version in which this event was introduced. |
...
| `tokens` | `object` | No | Token overrides: keys are purposes, values are token identifiers. |
...
Every link requires a `type` and at least one of `url` (for external resources) or `identifier` (for internal DSDS entity references). Both can be present when an internal entity also has an addressable URL.
...
| `identifier` | `string` | Conditional | Identifier of a referenced DSDS entity. Required when `url` is not provided. |
