...
The `kind` field is named for RFC 2119 requirement levels:

| Value | Meaning |
|---|---|
| `MUST` | Required — non-compliance is a defect. |
| `SHOULD` | Follow in most cases; exceptions require justification. |
| `NEUTRAL` | Advisory only, with no enforcement expectation (the default). |
...
The purpose document block type wraps the common `useCases` data model with a `kind` discriminator. `useCases` is an **array** of scenario entries, each with a `description`, a `kind` of `"positive"` (when to use) or `"negative"` (when to choose something else), and — for negative cases — an `alternative`:
...
In both, `useCases` is an **array** of scenario entries. Each entry has a `description` and a `kind` of `"positive"` (when to use) or `"negative"` (when to choose something else); negative entries SHOULD include an `alternative` pointing to a more fitting entity:
