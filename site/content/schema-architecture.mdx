...
Patterns reference their participating components through the `links` array (with `kind: "component"`, `identifier`, and `role`) instead of through a dedicated guideline type.
...
The *kind* of guide (getting-started, contribution, tutorial, concept, migration) is expressed through a `category` metadata entry, keeping the entity flexible enough for any guideline.
