...
DSDS {{VERSION}} is a **pre-1.0 draft**. This page is the contract around that status: what can change and how you will be told, what will never change, the complete list of known breaking changes still to come, and the conditions under which 1.0 is declared. The goal is that an adopter can read this page and price the risk of adopting now.
...
| 7 | Remove `link`'s deprecated inter-entity surface | validates with DSDS-010 warning | removed; `relationships` only |
| 8 | Remove chunk's top-level `guidelines`/`useCases` | validates, marked deprecated | removed; `documentBlocks` only |
| 9 | Graduate draft metadata fields | `governance` / `docOrigin` marked "initial draft" | shapes frozen |
