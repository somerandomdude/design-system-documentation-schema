...
0.15.0 is a minor release. It renames nothing and adds three constraints, so most documents migrate with only a `dsdsVersion` bump. There is no migration script — the affected positions are few and the edits are specific. Fix these if your documents hit them:
...
0.14.0 is a breaking release. It executed every recorded naming exception and removed both deprecated surfaces. Documents at 0.10 through 0.13 migrate mechanically.
...
- **Identifier-bearing links.** A link that pointed at another entity by `identifier` no longer validates. The migrator reports each one instead of guessing. Convert it to a typed `relationships` edge (`composes`, `depends-on`, `alternative-to`, …) or rewrite it as a plain URL link.
- **Chunks that had both forms.** When a chunk carried the shorthand *and* an equivalent block, the block was authoritative. The migrator merges the shorthand items it does not already contain and reports the counts. Skim the report to confirm nothing merged that you meant to drop.
