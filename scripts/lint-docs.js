...
const IMPLEMENTATIONS = {
  ...
  "entity-missing-use-cases": (entity, pointer, emit) => {
    ...
    if (
      ...
      !blocks.some((b) => b && b.kind === "use-cases")
    ) {
      emit(
        pointer,
        `${entityLabel(entity)} has no use-cases block — "when do I use this?" is the first question documentation must answer. Add at least one recommended scenario.`,
      );
    }
  },
